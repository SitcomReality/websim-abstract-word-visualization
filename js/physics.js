...
export class Physics {
                ...
                if (distSq < minDistSq && distSq > 1e-6) { 
                    ...
                    if (now - this.lastCollisionTime > this.collisionCooldown) {
                        ...
                        if (impactSpeed > 1) { 
                            ...
                            // Check if collision generates energy (based on engine multiplier)
                            if (impactSpeed > 3 && this.engine.physics.collisionEnergyMultiplier > 1) {
                                const collisionEnergy = Math.floor(impactSpeed * this.engine.physics.collisionEnergyMultiplier);
                                if (collisionEnergy > 0) {
                                    this.engine.addEnergy(collisionEnergy);
                                    this.showCollisionEnergyEffect(contactX, contactY, collisionEnergy);
                                }
                            }
                        }
                    }
    ...
    showCollisionEnergyEffect(x, y, amount) {
        ...
        energyText.style.left = `${x}px`;
        energyText.style.top = `${y}px`;
        energyText.style.color = '#4caf50';
        energyText.style.fontWeight = 'bold';
        energyText.style.textShadow = '0 0 5px rgba(0,0,0,0.8)';
        energyText.style.pointerEvents = 'none';
        energyText.style.zIndex = '100';
        ...
        energyText.animate([
            ...
            if (this.engine.container.contains(energyText)) {
                this.engine.container.removeChild(energyText);
            }
        };
    }
}